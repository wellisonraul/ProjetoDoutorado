...
import ee.tkasekamp.ltlminer.rulecreator.ActivityCombiner;
...
public class LTLMiner {
	...
	public ArrayList<RuleModel> mine(XLog log, ArrayList<String> ruleTemplates, double minSupport) {
		...
		ArrayList<String> resources = logFilter.getAllResource(log);		
		System.out.println("Os resouces existente nesse log  são : ");
		for(String aux : resources){
			System.out.println("resource :"+ aux);
		}
	...
	public ArrayList<RuleModel> mineWithEventTypes(XLog log, ArrayList<String> ruleTemplates,double minSupport) {
		this.minSupport = minSupport;
		ArrayList<String> events = logFilter.getAllEvents(log);
		System.out.println(events.size());
		ArrayList<String> eventTypes = logFilter.getEventTypes(log);
		System.out.println(eventTypes.get(0));
	...
	public ArrayList<RuleModel> mine(XLog log, Properties properties) {
		...
		boolean considerEventTypes = Boolean.parseBoolean(properties.getProperty("considerEventTypes"));

		ArrayList<String> events = logFilter.getAllEvents(log);
		ArrayList<String> res = logFilter.getAllResource(log);
		if(res.get(0)==null){
			res.remove(0);//ALTEREI AQUIIIIIIIIII			
		}
		events.addAll(res);
	...
	public Object[] checkRules(XLog log, ArrayList<String> rules) {
	 ...
	 * @return
	 */
	public ArrayList<RuleModel> filter(Object[] objList) {
