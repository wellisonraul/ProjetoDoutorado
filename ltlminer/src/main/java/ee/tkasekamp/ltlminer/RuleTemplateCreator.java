...
public class RuleTemplateCreator {
	...
	private static String doSomething(String query) {
		...
		while (m.find()) {
			...
			if(parameter.substring(0, 3).equals("res")){
				 //System.out.println("resource == " + parameter);
				 text = "resource == " + parameter;
			}
			if(parameter.substring(0, 3).equals("act")){
				//System.out.println("activity == " + parameter);
				 text = "activity == " + parameter;
			}
			if(parameter.substring(0, 3).equals("typ")){
				//System.out.println("event == " + parameter);
				text = "eventtype == " + parameter;
			}
			
		//	String text = "resource == " + parameter;
